...
$error_messages = array(
	...
	78 => 'unknown error getting token'
);
...
function get_token() {
	...
	$unknown_error = 78;
	if ((!$conn = get_db_conn()) |
		(!$stmt = $conn->prepare('CALL get_token(?, ?, @error_19, @token)')) |
		(!$stmt->bind_param('ss', $username, $pwd)) |
		(!$stmt->execute()) |
		(!$stmt->close()) |
		(!$result_19 = $conn->query('SELECT @error_19')) |
		(!$result_token = $conn->query('SELECT @token')) |
		(!$conn->close())
	) {
		$response_body_array['errors'][] = $unknown_error;
		echo(prepare_response_body($response_body_array));
	...
	$response_body_array['token'] = $result_token->fetch_assoc()['@token'];
	$result_token->free();
